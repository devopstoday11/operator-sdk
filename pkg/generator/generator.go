...
func renderBuildFiles(buildDir, repoPath, projectName, operatorType string, generatePlaybook bool) error {
	...
	// Always generate Docker build files
	buf = &bytes.Buffer{}
	...
	if err := renderFile(buf, "tmp/build/Dockerfile", dockerFileTmplName, dTd); err != nil {
		return err
	}
	return renderWriteFile(filepath.Join(buildDir, dockerfile), "tmp/build/Dockerfile", dockerFileTmplName, dTd)
}
